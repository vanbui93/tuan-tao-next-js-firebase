...
export default function SearchResult(props) {
    ...
    const getThumbnail = (imgThumb) => {
        ...
        return (
            <img src={img[0]} alt='/' loading="lazy" className='search-result__img' />
        )
    }

    return (
        <div className='search-result'
            onClick={() => { //Đóng kết quả search khi click chọn xem kết quả search
                toggleModal();
            }}
            ref={wrapperRef}
        >
                    ...
                    result?.map((val, index) => {
                        ...
                        return (
                            <li className='search-result__item' key={index}>
                                <Link to={`/product/${link}`} className='search-result__link'>
