...
export default function ProductItem(props) {
    ...
    const img = [];
    images !== null && images !== undefined && Object.values(images)?.map((item) => {
        if (item !== null) {
            img.push(item)
        }
    });
    ...
    const getPromoDefault = (promoDefault) => {
        const promo = [];
        promoDefault !== undefined && Object.values(promoDefault)?.map((item) => {
            if (item !== null) {
                promo.push(item);
            }
        });
    ...
    const ckPromotionIds = promotions !== null && promotions !== undefined && promotions.length && promotions?.map((item) => {
        if (item !== null) {
    ...
    return (
        <li className="collections__item">
            <Link to={`/product/${id}`} className='collection__link' onClick={() => window.scrollTo(0, 0)}>
                <div className='collections__img'><img src={img[0]} alt="" /></div>
                <div className="collections__sticker">
                    {
                        newPercent ? <p className='collections__percent-pin'><span>new: {newPercent}%</span></p> : ''
                    }
                ...
                <div className="collections__info">
                    ...
                    <p className="collections__price">
                        <strong className='collections__new-price'>{price ? `${numberWithCommas(price)} đ` : `Liên hệ: ${mainData?.phone}`}</strong>
                        {price && comparePrice && <strike className='collections__compare-price'>{numberWithCommas(comparePrice)} đ</strike>}
                    </p>
                </div>
                {
                    ckPromotionIds ?
