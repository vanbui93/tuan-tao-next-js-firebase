...
import { deleteUser, getUser, updateUser } from 'actions/account';
...
const AdminUsers = (props) => {
  ...
  const users = useSelector(state => state.account.data);
  ...
  const arrayUser = [];
  users !== null && users !== undefined && Object.keys(users)?.map((element) => {
    const key = element;
    if (users[key] !== null) {
  ...
  const handleAddAccount = () => {
    navigate("/dashboard/user_add", { replace: true })
  }
          ...
          <div>
            ...
            <TableContainer component={Paper}>
              <Table sx={{ minWidth: 700 }} aria-label="customized table">
                ...
                <TableBody>
                  {arrayUser !== null && arrayUser !== undefined &&
