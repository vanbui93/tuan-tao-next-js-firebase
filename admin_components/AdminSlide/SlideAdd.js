...
import { addSlideObject, getSlides } from 'actions/slides';
...
function SlideAdd(props) {
  ...
  const handleSaveSlide = async () => {
    try {
      ...
      dispatch(getSlides());
      navigate("/dashboard/slide", { replace: true })
    }
    catch (err) {
      console.log(err);
    }
    ...
    <AdminStyle open={!opensidebar}>
      <Grid>
          ...
          <Table>
            {
              <TableBody>
                <TableRow>
                  <TableCell className={classes.tbHeadLeft} variant="head">Upload hình</TableCell>
                  <TableCell>
                    <div className={classes.imgList}>
                      {result && <img className={classes.thumbnail} ref={imageRef} src={result} alt="" />}

                      <div className={`${classes.imgListItem} ${classes.imgListItemBlock}`}>
        ...
        <Stack spacing={2} direction="row" style={{ paddingTop: '20px' }}>
          ...
          <Button variant="contained" color="secondary" onClick={handleSaveSlide}>Lưu</Button>
        </Stack>
      </Grid>
    </AdminStyle>
...
export default withStyles(styles)(SlideAdd)