...
import { deleteSlide, getSlides, updateSlide } from 'actions/slides';
...
const AdminSlide = (props) => {
  ...
  const arraySlide = [];
  allSlides !== null && allSlides !== undefined && Object.keys(allSlides)?.map((element) => {
    const key = element;
    if (allSlides[key] !== null) {
      const img_url = allSlides[key].image_url ? allSlides[key].image_url : '';
      const text = allSlides[key].text ? allSlides[key].text : '';
      arraySlide.push({
        id: key,
        image_url: img_url,
        text: text
  ...
  const handleAddSlide = () => {
    navigate("/dashboard/slide_add", { replace: true })
  }
  ...
  const idSlideRef = useRef();
  const handleDelete = (id) => {
    handleDialog('Bán có chắc chắn muốn xóa không ?', true);
    idSlideRef.current = id;
  }

  const handleEditSlide = (slide) => {
    idSlideRef.current = slide.id;
    setIsEdit(true);
    setEditSlideObject(slide);
  }
        ...
        !isEdit ?
          <div>
            ...
            <TableContainer component={Paper}>
              <Table sx={{ minWidth: 700 }} aria-label="customized table">
                <TableHead>
                  <TableRow>
                    <StyledTableCell align="left">Hình slide</StyledTableCell>
                ...
                <TableBody>
                  {
                    arraySlide !== null && arraySlide !== undefined &&
                    Object.values(arraySlide)?.map((item, idx) => (
                      item &&
                      <StyledTableRow key={idx}>
                        <StyledTableCell className={classes.thumbnail}>
                          <img src={item.image_url ? item.image_url : ''} alt='/' loading="lazy" />
                        </StyledTableCell>
                        <StyledTableCell align="left">{item.text}</StyledTableCell>
          ...
          <Grid>
            <TableContainer component={Paper}>
              <Table>
                {
                  editSlideObject !== null && editSlideObject !== undefined &&
                  <TableBody>
                    <TableRow>
                      <TableCell className={classes.tbHeadLeft} variant="head">Hình slide</TableCell>
                      <TableCell>
                        <Grid container spacing={2}>
                          <Grid item xs={8}>
                            <img className={classes.thumbnail} src={editSlideObject.image_url} alt='/' loading="lazy" />
                          </Grid>
                          <Grid item xs={4}>
                    ...
                    <TableRow>
                      ...
                      <TableCell>
                        <TextField
                          ...
                          size="small"
                          fullWidth
                          defaultValue={editSlideObject.text}
                          name="text"
...
export default withStyles(styles)(AdminSlide)