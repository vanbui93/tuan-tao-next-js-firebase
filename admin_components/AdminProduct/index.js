import { Button, Checkbox, Fab, FormControl, Grid, MenuItem, Radio, RadioGroup, Select, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, withStyles } from '@material-ui/core';
...
import { deleteProduct, getProduct, updateImgProduct, updateProduct } from 'actions/products';
import { getPromotions } from 'actions/promotions';
import { getSkus } from 'actions/skus';
import { getVideo } from 'actions/videos';
import { getWarantys } from 'actions/warantys';
...
import PaginationButtons from 'Admin/components/Pagination';
...
const AdminProduct = (props) => {
  ...
  const arrayProduct = [];
  products !== null && products !== undefined && Object.keys(products)?.map((element) => {
    const key = element;
  ...
  const handleEditOnchage = (e) => {
    ...
    console.log(editObject.isDisplay);
  }
  ...
  const handleAddProduct = () => {
    navigate("/dashboard/product_add", { replace: true })
  }

  //Kiểm tra màu có nằm trong danh sách màu hay không
  const ckColorIds = editObject.colors?.length && editObject.colors.map((item) => {
    if (item !== null) {
  ...
  const handleChangeColor = (colorId) => (e) => {
    if (ckColorIds?.includes(colorId)) {
      ...
      const newArrWithRemovedColor = editObject.colors.length &&
      ...
      const colorIndex = allColors !== null && allColors !== undefined &&
        Object.values(allColors)?.filter(
          color => color.color_id === colorId
        );

      const newArrWithAddedColor = [
        ...editObject.colors,
        { color_id: colorIndex[0].color_id }
      ];
  ...
  const ckSkuIds = editObject.skus?.length && editObject.skus.map((item) => {
    if (item !== null) {
      return item.sku_id;
    }
  })
  ...
  const handleChangeSku = (skuId) => (e) => {
    if (ckSkuIds?.includes(skuId)) {
      ...
      const newArrWithRemovedSku = editObject.skus.length &&
      ...
      const skuIndex = allSkus !== null && allSkus !== undefined &&
      ...
      const newArrWithAddedSku = [
        ...editObject.skus,
        { sku_id: skuIndex[0].sku_id }
      ];
  ...
  const ckVideoIds = editObject.videos?.length && editObject.videos.map((item) => {
    if (item !== null) {
  ...
  const handleChangeVideo = (videoId) => (e) => {
    if (ckVideoIds?.includes(videoId)) {
      ...
      const newArrWithRemovedVideo = editObject.videos.length &&
      ...
      const videoIndex = allVideos !== null && allVideos !== undefined &&
        Object.values(allVideos)?.filter((video) => {
          if (video) {
            return video.video_id === videoId
          }
        });
      const newArrWithAddedVideo = [
        ...editObject.videos,
        { video_id: videoIndex[0].video_id }
      ];
  ...
  const ckWarantyIds = editObject.warantys?.length && editObject.warantys.map((item) => {
    if (item !== null) {
  ...
  const handleChangeWaranty = (warantyId) => (e) => {
    if (ckWarantyIds?.includes(warantyId)) {
      ...
      const newArrWithRemovedWaranty = editObject.warantys.length &&
      ...
      const warantyIndex = allWarantys !== null && allWarantys !== undefined &&
      ...
      const newArrWithAddedWaranty = [
        ...editObject.warantys,
        { waranty_id: warantyIndex[0].waranty_id }
      ];
  ...
  const ckPromotionIds = editObject.promotions?.length && editObject.promotions?.map((item) => {
    if (item !== null) {
  ...
  const handleChangePromotion = (promotionId) => (e) => {
    if (ckPromotionIds?.includes(promotionId)) {
      ...
      const newArrWithRemovedPromotion = editObject.promotions.length &&
      ...
      const promotionIndex = allPromotions !== null && allPromotions !== undefined &&
  ...
  //Phân trang
  const allList = [...arrayProduct].sort((a, b) => new Date(b.create_date) - new Date(a.create_date) || new Date(b.update_date) - new Date(a.update_date));
                  ...
                  {currentList !== null && currentList !== undefined &&
                    Object.values(currentList)?.map((product, idx) => (
                      <StyledTableRow key={idx}>
                        <StyledTableCell className={classes.tbNameStyle}>{product?.name}</StyledTableCell>
                        <StyledTableCell align="left" className={classes.thumbnail}>
                          {
                            getImgThumb(product?.images)
                          }
                        </StyledTableCell>
                        <StyledTableCell>{product?.category === 1 ? 'iPhone' : 'Phụ kiện'}</StyledTableCell>
                        <StyledTableCell>{product?.fullbox === 1 ? 'Đã sử dụng' : 'Mới fullbox'}</StyledTableCell>
                        <StyledTableCell align="left">{product?.price ? `${numberInputFormat(product?.price.toString())} đ` : 'Liên hệ'} </StyledTableCell>
                        ...
                        <StyledTableCell align="left">{product?.isDisplay === '1' ? 'Hiển thị' : 'Ẩn'}</StyledTableCell>
              ...
              <Table>
                {
                  ...
                  <TableBody>
                    <TableRow>
                      <TableCell className={classes.tbHeadLeft} variant="head">Tên sản phẩm</TableCell>
                      <TableCell>
                        <TextField
                    ...
                    <TableRow>
                      <TableCell className={classes.tbHeadLeft} variant="head">Hình sản phẩm</TableCell>
                      <TableCell>
                        <ul className={classes.imgList}>
                          {
                            imgsSrc !== null && imgsSrc !== undefined &&
                            Object.values(imgsSrc)?.map((link, idx) => {
                              if (link !== null && link !== undefined) {
                                return (
                                  <li className={classes.imgListItem} key={idx}>
                                    <img src={link} alt='/' loading="lazy" />
                                    <span className={classes.deleteImg}
                                      onClick={() => setImgsSrc(imgsSrc.filter((e) => e !== link))}
                                    >
                                      <DeleteIcon />
                                    </span>
                                  </li>
                    ...
                    </TableRow>
                    {
                      ckColorIds ?
                        <TableRow>
                          <TableCell className={classes.tbHeadLeft} variant="head">Màu sắc</TableCell>
                          <TableCell>
                            {
                              allColors !== null && allColors !== undefined &&
                              Object.values(allColors)?.map((ckColor, idx) => (
                                ckColor &&
                                <FormControlLabel
                                  key={idx}
                                  ...
                                  control={
                                    <Checkbox
                                      defaultChecked={ckColorIds?.includes(ckColor.color_id)}
                                      name='ckColor'
                    ...
                    }
                    {
                      ckSkuIds ?
                        <TableRow>
                          ...
                          <TableCell>
                            {
                              allSkus !== null && allSkus !== undefined &&
                              Object.values(allSkus)?.map((ckSku, idx) => (
                                ckSku &&
                                <FormControlLabel
                                  key={idx}
                                  label={ckSku.memory}
                                  control={
                                    <Checkbox
                                      defaultChecked={ckSkuIds?.includes(ckSku.sku_id)}
                                      name='ckSku'
                                      color="primary"
                    ...
                    <TableRow>
                      <TableCell className={classes.tbHeadLeft} variant="head">Giá sản phẩm</TableCell>
                      <TableCell>
                        <Grid container spacing={2}>
                          <Grid item xs={2}>
                            <TextField
                              ...
                              size="small"
                              fullWidth
                              defaultValue={numberInputFormat(editObject.price.toString())}
                              name="price"
                              onChange={handleEditOnchage}
                            />
                          </Grid>
                          ...
                          <Grid item xs={2}>
                            <TextField
                              ...
                              size="small"
                              fullWidth
                              defaultValue={numberInputFormat(editObject.compare_price.toString())}
                              name="compare_price"
                              onChange={handleEditOnchage}
                            />
                          </Grid>
                    ...
                    <TableRow>
                      <TableCell className={classes.tbHeadLeft} variant="head">Nhóm sản phẩm</TableCell>
                      <TableCell>
                        <FormControl>
                          <Select
                    ...
                    <TableRow>
                      <TableCell className={classes.tbHeadLeft} variant="head">New</TableCell>
                      <TableCell>
                        <Grid container spacing={2}>
                          <Grid item xs={2}>
                            <TextField
                    ...
                    </TableRow>
                    {
                      ckWarantyIds ?
                        <TableRow>
                          <TableCell className={classes.tbHeadLeft} variant="head">Bảo hành</TableCell>
                          <TableCell>
                            {
                              allWarantys !== null && allWarantys !== undefined &&
                              Object.values(allWarantys)?.map((ckWaranty, idx) => (
                                <Grid key={idx}>
                                  <FormControlLabel
                                    ...
                                    control={
                                      <Checkbox
                                        defaultChecked={ckWarantyIds?.includes(ckWaranty.waranty_id)}
                                        name='ckWaranty'
                                        color="primary"
                    ...
                    <TableRow>
                      <TableCell className={classes.tbHeadLeft} variant="head">FullBox ?</TableCell>
                      <TableCell>
                        <FormControl>
                          <Select
                            labelId="demo-simple-select-label"
                    ...
                    </TableRow>
                    {
                      ckPromotionIds ?
                        <TableRow>
                          <TableCell className={classes.tbHeadLeft} variant="head">Khuyến mãi</TableCell>
                          <TableCell>
                            {
                              allPromotions !== null && allPromotions !== undefined &&
                              Object.values(allPromotions)?.map((ckPromotion, idx) => (
                                ...
                                <FormControlLabel
                                  ...
                                  control={
                                    <Checkbox
                                      defaultChecked={ckPromotionIds?.includes(ckPromotion.promotion_id)}
                                      name='ckPromotion'
                                      color="primary"
                                      onChange={handleChangePromotion(ckPromotion.promotion_id)}
                                    />
                                  }
                                />
                              ))
                    ...
                    }
                    {
                      ckVideoIds ?
                        <TableRow>
                          <TableCell className={classes.tbHeadLeft} variant="head">Video</TableCell>
                          <TableCell>
                            {
                              allVideos !== null && allVideos !== undefined &&
                              Object.values(allVideos)?.map((ckVideo, idx) => (
                                ckVideo &&
                                < FormControlLabel
                                  key={idx}
                                  ...
                                  control={
                                    <Checkbox
                                      defaultChecked={ckVideoIds?.includes(ckVideo.video_id)}
                                      name='ckVideo'
                                      color="primary"
                    ...
                    <TableRow>
                      <TableCell className={classes.tbHeadLeft} variant="head">Hiển thị</TableCell>
                      <TableCell>
                        <FormControl>
                          <RadioGroup
                            row
