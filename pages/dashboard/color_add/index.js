...
import { addColorObject, getColors } from 'actions/colors';
...
function ColorAdd(props) {
  ...
  const handleSaveColor = async () => {
    try {
      ...
      dispatch(getColors());
      navigate("/dashboard/color", { replace: true })
    }
    catch (err) {
      console.log(err);
    }
    ...
    <AdminStyle open={!opensidebar}>
      ...
      <Grid>
          ...
          <Table>
            {
              <TableBody>
                <TableRow>
                  <TableCell className={classes.tbHeadLeft} variant="head">Tên màu</TableCell>
                  <TableCell>
                    <TextField
        ...
        <Stack spacing={2} direction="row" style={{ paddingTop: '20px' }}>
          ...
          <Button variant="contained" color="secondary" onClick={handleSaveColor}>Lưu</Button>
        </Stack>
      </Grid>
    </AdminStyle>
